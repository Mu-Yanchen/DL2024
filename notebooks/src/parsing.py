...
class Parser:
    ...
    def _extract_labels_and_bboxes_from_html(self, predition: str):
        ...
        if not (len(labels) == len(x) == len(y) == len(w) == len(h)):
            raise RuntimeError
        labels = [label for label in labels if label!='pagination']
    ...
    def _extract_labels_and_bboxes_from_seq(self, prediction: str):
        ...
        labels = torch.tensor([self.label2id[label] if label!='background_image' else self.label2id['background'] for label in labels])
        bboxes = torch.tensor(
            [
                [
...
class Parser_HF:
    ...
    def _extract_labels_and_bboxes_from_html(self, predition: str):
        ...
        if not (len(labels) == len(x) == len(y) == len(w) == len(h)):
            raise RuntimeError
        labels = [label for label in labels if label!='pagination']
        labels = [self.label2id[label] if label!='background_image' else self.label2id['background'] for label in labels]
        bboxes = [
                [
                    int(x[i]) / self.canvas_width,
                    int(y[i]) / self.canvas_height,
